...
**SimpleAdo.Sqlite** is a .NET Standard library, and requires .NET Standard 1.1 compliance by the platform running it. For developers unfamiliar with .NET Standard libraries, it may be helpful to think of it as a new style of Portable Class Library (PCL); you consume the library the same way that you would have consumed PCLs in your projects in the past. At the time of writing, .NET Standard 1.1 compliance means that it should be compatible with the following platforms - with updated information available [here](https://docs.microsoft.com/en-us/dotnet/standard/net-standard):
...
---------------
  ...
  3. The latest released version of this library is available via NuGet. From within Visual Studio and Xamarin Studio, search for **SimpleAdo.Sqlite**
  ...
  5. This library **does not implement full database encryption** - for that, please investigate SQLCipher - available [here](http://sqlcipher.net/). That functionality may also possible by using a special version of the **SQLitePCL.raw** library, but this option is currently unexplored.
